...
export async function GET() {
  const now = new Date();
  const nowMs = now.getTime(); // ⚡ Bolt: Cache current time for faster comparisons
  const tasks = getTasks();
  const team = getTeamMembers();

  // ⚡ Bolt: Use a Map for O(1) team member lookups, reducing complexity from O(N*M) to O(N+M)
  const teamMap = new Map(team.map((m) => [m.id, m]));

  const results: { task: string; type: string; sent: boolean }[] = [];
  let updated = false;

  // ⚡ Bolt: Collect all pending notifications across all tasks for maximum parallelization
  const allPendingNotifications: Promise<void>[] = [];
  ...
  // ⚡ Bolt: Execute all notifications in parallel to reduce total execution time to ~max single request latency
  if (allPendingNotifications.length > 0) {
    await Promise.allSettled(allPendingNotifications);
  }

  // ⚡ Bolt: Only update storage if notifications actually succeeded
