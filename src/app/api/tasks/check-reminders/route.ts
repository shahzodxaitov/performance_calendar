 ...
 * to allow for conditional state updates in batch operations.
 */
...
export async function GET() {
  // ⚡ Bolt: Use numeric timestamp once for all comparisons to avoid repeated Date object creation
  const nowMs = Date.now();
  const tasks = getTasks();
  const team = getTeamMembers();

  // ⚡ Bolt: Convert team to Map for O(1) lookups inside the loop (Overall O(N + M) vs O(N * M))
  ...
  for (const task of tasks) {
    ...
    // ⚡ Bolt: Only instantiate Date when needed for calculation
