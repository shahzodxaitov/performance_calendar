...
/**
 * ⚡ Bolt: Added return boolean to track success
 */
...
export async function GET() {
  ...
  // ⚡ Bolt: O(N+M) complexity by using a Map for team member lookups
  ...
  for (const task of tasks) {
    ...
    // We still need Date for parsing complex ISO strings with TZ,
    // but we use the hoisted nowMs for comparison.
    ...
    if (!task.notified_1day && hoursLeft > 0 && hoursLeft <= 28 && hoursLeft > 2) {
      ...
      // ⚡ Bolt: Parallelize notifications and only update state on success
      notifications.push(
  ...
  // ⚡ Bolt: Wait for all parallel notifications to finish
