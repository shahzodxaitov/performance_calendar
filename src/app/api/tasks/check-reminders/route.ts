...
/**
 * ⚡ Bolt: Added success indicator to ensure task flags are only updated on successful delivery.
 */
...
export async function GET() {
  /**
   * ⚡ Bolt Optimizations:
   * 1. Hoisted reference timestamp (nowMs) to ensure consistency and avoid redundant Date object creations.
   * 2. Map-based member lookup reduces complexity from O(N*M) to O(N+M).
   * 3. Parallelized notifications using Promise.allSettled to eliminate O(T) sequential network latency.
   * 4. Success-based flag updates ensure state integrity.
   */
  const nowMs = Date.now();
  const tasks = getTasks();
  const team = getTeamMembers();

  // O(M) Map construction
  ...
  // Parallelize all pending notifications
