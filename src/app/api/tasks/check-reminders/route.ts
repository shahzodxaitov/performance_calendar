...
export async function GET() {
  // ⚡ Bolt: Use a single baseline timestamp for consistent batch processing.
  const now = new Date();
  const nowMs = now.getTime();
  const tasks = getTasks();
  const team = getTeamMembers();

  // ⚡ Bolt: O(N+M) complexity by using a Map for team member lookups instead of O(N*M).
  ...
  for (const task of tasks) {
    ...
    // ⚡ Bolt: Combine date and time once per task for comparison.
  ...
  // ⚡ Bolt: Parallelize all pending notifications to optimize execution time from O(N*L) to O(L).
  if (notifications.length > 0) {
    await Promise.allSettled(notifications);
  }

  // ⚡ Bolt: Persist state changes only if something actually updated.
  if (updated) {
    saveTasks(tasks);
  }
