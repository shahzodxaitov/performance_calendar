...
export async function GET() {
  // ⚡ Bolt: Cache baseline timestamp once to ensure consistency
  ...
  // ⚡ Bolt: Collect notifications to process in parallel
  const notifications: Promise<{ success: boolean, task: LocalTask, type: string }>[] = [];

  for (const task of tasks) {
    // ⚡ Bolt: Early exit for done or already fully notified tasks
    ...
    if (!task.notified_1day && hoursLeft > 0 && hoursLeft <= 28 && hoursLeft > 2) {
      ...
      notifications.push(
        sendTelegramNotification(member.chat_id, message)
          .then(success => ({ success, task, type: "1day" }))
      );
    }
    ...
    if (!task.notified_1hour && hoursLeft > 0 && hoursLeft <= 1.5) {
      ...
      notifications.push(
        sendTelegramNotification(member.chat_id, message)
          .then(success => ({ success, task, type: "1hour" }))
      );
    }
  }

  // ⚡ Bolt: Process notifications in parallel using Promise.allSettled
