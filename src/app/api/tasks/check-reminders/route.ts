 ...
 * to ensure state updates only occur on successful notification delivery.
 */
...
export async function GET() {
  const now = new Date();
  const nowMs = now.getTime(); // ⚡ Bolt: Cache baseline timestamp for consistency
  const tasks = getTasks();
  const team = getTeamMembers();

  // ⚡ Bolt: Replace O(N*M) linear search with O(N+M) Map for performance
  ...
  for (const task of tasks) {
    ...
    // ⚡ Bolt: Using numeric timestamps for faster arithmetic
  ...
  // ⚡ Bolt: Parallelize all notifications to minimize total request latency
