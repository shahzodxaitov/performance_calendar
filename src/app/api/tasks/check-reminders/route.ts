...
/**
 * ⚡ Bolt: Optimized Telegram notification helper to return success status.
 */
...
export async function GET() {
  // ⚡ Bolt: Hoist current timestamp to ensure consistent comparison across all tasks
  const now = new Date();
  ...
  // ⚡ Bolt: Use a Map for O(1) team member lookups instead of O(M) .find() inside the loop
  // Complexity reduced from O(N*M) to O(N+M)
  ...
  for (const task of tasks) {
    ...
    // ⚡ Bolt: Optimized deadline calculation
  ...
  // ⚡ Bolt: Wait for all notifications to complete
