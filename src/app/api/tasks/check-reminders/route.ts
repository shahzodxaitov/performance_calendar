...
// ⚡ Bolt: Helper returns boolean to safely track success in parallel batches
...
export async function GET() {
  const nowMs = Date.now(); // ⚡ Bolt: Use numeric timestamp for O(1) arithmetic
  const tasks = getTasks();
  const team = getTeamMembers();

  // ⚡ Bolt: Use Map for O(1) lookup, reducing complexity from O(N*M) to O(N+M)
  ...
  for (const task of tasks) {
    ...
    // ⚡ Bolt: Pre-calculate deadline timestamp once per task
  ...
  // ⚡ Bolt: Parallelize all notifications to avoid sequential latency bottlenecks
