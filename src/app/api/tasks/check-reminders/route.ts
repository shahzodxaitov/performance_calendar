...
export async function GET() {
  ...
  // ⚡ Bolt: Use Map for O(1) team member lookup to avoid O(N*M) complexity in the loop
  ...
  for (const task of tasks) {
    ...
    // Use consistent timezone offset for local calculations
    ...
    if (!task.notified_1day && hoursLeft > 0 && hoursLeft <= 28 && hoursLeft > 2) {
      ...
      // ⚡ Bolt: Parallelize notifications to avoid blocking the loop with network latency
