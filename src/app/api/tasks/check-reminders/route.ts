...
export async function GET() {
  ...
  /**
   * ⚡ Bolt: Optimization - Complexity Reduction
   * Replacing O(N*M) nested lookup with O(N+M) Map for O(1) member access.
   */
  ...
  for (const task of tasks) {
    ...
    /**
     * ⚡ Bolt: Optimization - Performance
     * Use numeric timestamps for faster arithmetic comparison.
     */
    ...
    const hoursLeft = diffMs / 3600000; // (1000 * 60 * 60)
  ...
  /**
   * ⚡ Bolt: Optimization - Concurrency
   * Parallelizing independent notification requests to reduce total execution time
